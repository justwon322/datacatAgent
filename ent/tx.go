...
func (tx *Tx) init() {
	tx.CategoryMain = NewCategoryMainClient(tx.config)
	tx.CategorySub = NewCategorySubClient(tx.config)
	tx.ExecutionLog = NewExecutionLogClient(tx.config)
	tx.Job = NewJobClient(tx.config)
	tx.Message = NewMessageClient(tx.config)
	tx.MessageMail = NewMessageMailClient(tx.config)
...
// applies a query, for example: CategoryMain.QueryXXX(), the query will be executed
